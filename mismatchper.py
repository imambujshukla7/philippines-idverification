...
def find_similar_parts(target, input_text, threshold=0.60):
  ...
  for i in range(len(words)):
    for j in range(i + 1, len(words) + 1):
      part = ' '.join(words[i:j])
      similarity = 1 - Levenshtein.distance(target, part) / max(
        len(target), len(part))
    ...
    for i in range(n_boxes):
      if int(extracts['conf'][i]) > 10:
        if re.match(pattern, extracts['text'][i]):
          (x, y, w, h) = (extracts['left'][i], extracts['top'][i],
                          extracts['width'][i], extracts['height'][i])
          coord_list.append(x)
...
def compare_center_allignment(part, strings, extracts):
  ...
  if abs(coordinate) <= 5 and len(coord_list) == 2 and abs(coord_list[0] -
                                                           coord_list[1]) >= 1:
    return True
  else:
    return False
...
def multyProcessImages(image_path):
  ...
  gussian_image = cv2.GaussianBlur(src=gray_image,
                                   ksize=(3, 3),
                                   sigmaX=0,
                                   sigmaY=0)
  ...
  _, thresold_image1 = cv2.threshold(gussian_image,
                                     thresh=165,
                                     maxval=255,
                                     type=cv2.THRESH_TRUNC + cv2.THRESH_OTSU)
  images.append(thresold_image1)

  _, thresold_image2 = cv2.threshold(gray_image,
                                     thresh=165,
                                     maxval=255,
                                     type=cv2.THRESH_TRUNC + cv2.THRESH_OTSU)
  images.append(thresold_image2)

  _, thresold_image3 = cv2.threshold(clached_image,
                                     thresh=165,
                                     maxval=255,
                                     type=cv2.THRESH_TRUNC + cv2.THRESH_OTSU)
  images.append(thresold_image3)

  _, thresold_image4 = cv2.threshold(unsharp_image,
                                     thresh=165,
                                     maxval=255,
                                     type=cv2.THRESH_TRUNC + cv2.THRESH_OTSU)
...
def fillStatus(image, left_list, centerd_list):
  global parameters
  extracts = pytesseract.image_to_data(image,
...
def findMissMatchPercentage(image_path):
  ...
  status = computeStatus(processedImages, left_aligned_list,
                         center_aligned_list)
...
if __name__ == "__main__":
  directory = input(
    "Please enter the path of the directory that contains the images:")
